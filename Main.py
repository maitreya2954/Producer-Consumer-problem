...
class Producer(threading.Thread):
    ...
    def run(self):
        # To create apple_count(Global Variable) in local context
        global apple_count
        while True:
            # Generate randomly a number to assign number to apple
            integer = random.randint(0, 40)

            # print 'condition acquired by %s' % self.name
            # When the Fridge has reached its capacity i.e the buffer is full
            if len(self.integers) == Capacity:
                ...
                (self.X_cord, apple_count) = Produce_Go_Right(self.X_cord, self.Y_cord,
                                                              self.producer_no, Lady_Image)  # Producer going right with an apple animation sequence
                self.condition.acquire()  # Acquiring lock on the Fridge
                # HAVE TO GO BACK WITH AN APPLE IN HAND
                self.X_cord = Producer_Go_Left_Full(
                    self.X_cord, self.Y_cord, self.producer_no, Lady_Image)
                ...
                # Append the apple number in fridge
                self.integers.append(integer)
                self.condition.acquire()
                (self.X_cord, apple_count) = Produce_Go_Right(self.X_cord, self.Y_cord,
                                                              self.producer_no, Lady_Image)  # producer goes right with an apple
                ...
                # producer puts apple in refrigerator and returns empty handed
                self.X_cord = Producer_Go_Left(
                    self.X_cord, self.Y_cord, self.producer_no, Lady_Image)
        ...
        while True:
            ...
            while True:
                if len(self.integers) == 0:  # If fridge is empty i.e no apples
                    ...
                    # pygame.mixer.music.play()
                    pygame.mixer.Channel(0).play(pygame.mixer.Sound(BGM))
                    self.X_cord = Consumer_Go_Left(
                        self.X_cord, self.Y_cord, self.consumer_no, Lady_Image)  # Consumer comes from left
                    # pygame.mixer.Channel(1).play(pygame.mixer.Sound(Nil))
                    self.condition.acquire()  # Acquiring lock on the fridge
                    self.X_cord = Consumer_Go_Right_Empty(
                        self.X_cord, self.Y_cord, self.consumer_no, Lady_Image)  # CONSUMER HAS TO GO BACK EMPTY HANDED\
                    self.condition.wait()  # Waits for Notify Signal
                    self.condition.release()  # Releasing the lock on Fridge
                    print('Empty ...')
                    f.write('Empty\n')
                ...
                if self.integers:
                    ...
                    self.X_cord = Consumer_Go_Left(
                        self.X_cord, self.Y_cord, self.consumer_no, Lady_Image)  # Consumer comes from left
                    print('%d removed from list by consumer(%s)' %
                          (integer, self.name))
                    ...
                    self.condition.wait()  # Waits for Notify Signal
                    self.condition.release()  # Releasing the lock on Fridge
                    # Consumer goes to right with an apple
                    self.X_cord = Consumer_Go_Right(
                        self.X_cord, self.Y_cord, self.consumer_no, Lady_Image)
