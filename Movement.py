...
# Setting up font details of the text
...
# Setting path for animation sequence of Producer Right
ProImageR = "./Images/Producer/GoRight/cycle10"
# Setting path for animation sequence of Producer Left
ProImageL = "./Images/Producer/GoLeft/cycle10"
# Setting path for animation sequence of Producer Left
ProImageLF = "./Images/Producer/GoLeftFull/cycle10"
Back = "./Images/Back2.jpg"  # Setting path background image

# Setting up path for consumer animaiton sequence (Right)
ConImageR = "./Images/Consumer/GoRight/Animated Walk and Skip Cycles-20"
# Setting up path for consumer animaiton sequence (Left)
ConImageL = "./Images/Consumer/GoLeft/consumer0"
# Setting up path for consumer animaiton sequence (Right)
ConImageRE = "./Images/Consumer/GoRightEmpty/consumer0"
...
def Consumer_Go_Right_Empty(X_cord, Y_cord, consumer_no, Lady_Image):
    ...
    while (go_right):
        ...
        if (X_cord < Consumer_Right_Extreme):
            # Load image of walking sequence
            Lady_Image = pygame.image.load(
                ConImageRE + str(image_id).zfill(2) + ".png")
            Lady_Image = pygame.transform.scale(
                Lady_Image, (368/CON_RATIO, 720/CON_RATIO))  # Scale the Lady_Image
            X_cord = X_cord + CON_SPEED  # Move the X coordinate for movement

        else:
            go_right = False
            # RENDER(TEXT,ANTI ALIASING,COLOR)
            Apple_text = myfont.render(
                'Apples: ' + str(apple_count), True, (173, 0, 0))
            Consumer_Number = myfont2.render(
                'C' + str(consumer_no), True, (173, 0, 0))
            ...
            gameDisplay.blit(Lady_Image, (X_cord, Y_cord)
                             )  # For drawing lady image
            gameDisplay.blit(Empty, (X_cord + 100, Y_cord + 50))
            # For drawing fridge
            gameDisplay.blit(Fridge, (Fridge_X_cord, Fridge_Y_cord))
            # For drawing the text that displays apple count
            gameDisplay.blit(
                Apple_text, (Apple_text_X_Cord, Apple_text_Y_Cord))
            gameDisplay.blit(Consumer_Number, (X_cord + Consumer_text_offset_x,
                             Y_cord + Consumer_text_offset_Y))  # For drawing the text of customer ID
...
def Consumer_Go_Right(X_cord, Y_cord, consumer_no, Lady_Image):
    ...
    while (go_right):
        ...
        if (X_cord < Consumer_Right_Extreme):
            # Load image of walking sequence
            Lady_Image = pygame.image.load(
                ConImageR + str(image_id).zfill(2) + ".png")
            Lady_Image = pygame.transform.scale(
                Lady_Image, (368/CON_RATIO, 720/CON_RATIO))  # Scale the Lady_Image
            X_cord = X_cord + CON_SPEED  # Move the X coordinate for movement
...
def Consumer_Go_Left(X_cord, Y_cord, consumer_no, Lady_Image):
    ...
    while (left):
        ...
        if (X_cord > Consumer_Left_Extreme):
            Lady_Image = pygame.image.load(
                ConImageL + str(image_id).zfill(2)+".png")
            Lady_Image = pygame.transform.scale(
                Lady_Image, (368/CON_RATIO, 720/CON_RATIO))
        ...
        gameDisplay.blit(Consumer_Number, (X_cord +
                         Consumer_text_offset_x, Y_cord + Consumer_text_offset_Y))
...
def Produce_Go_Right(X_cord, Y_cord, producer_no, Lady_Image):
    ...
    while (go_right):
        ...
        if (X_cord < Producer_Right_Extreme):
            Lady_Image = pygame.image.load(
                ProImageR+str(image_id).zfill(2)+".png")
            Lady_Image = pygame.transform.scale(
                Lady_Image, (368/PROD_RATIO, 720/PROD_RATIO))
            X_cord = X_cord + PROD_SPEED
        else:
            go_right = False
        # print(X_cord)
        # RENDER(TEXT,ANTI ALIASING,COLOR)
        # Text to display the count of apples
        Apple_text = myfont.render(
            'Apples: ' + str(apple_count), True, (173, 0, 0))
        Producer_Number = myfont3.render(
            'P' + str(producer_no), True, (173, 0, 0))
        gameDisplay.fill(white)
        ...
        gameDisplay.blit(Fridge, (Fridge_X_cord, Fridge_Y_cord))
        gameDisplay.blit(Apple_text, (Apple_text_X_Cord, Apple_text_Y_Cord))
        gameDisplay.blit(Producer_Number, (X_cord +
                         Producer_text_offset_x, Y_cord + Producer_text_offset_Y))
        pygame.display.update()
...
def Producer_Go_Left_Full(X_cord, Y_cord, producer_no, Lady_Image):
    ...
    while (left):
        ...
        if (X_cord >= Producer_Left_Extreme):
            Lady_Image = pygame.image.load(
                ProImageLF+str(image_id).zfill(2)+".png")
            Lady_Image = pygame.transform.scale(
                Lady_Image, (368/PROD_RATIO, 720/PROD_RATIO))
            X_cord = X_cord - PROD_SPEED
        else:
...
def Producer_Go_Left(X_cord, Y_cord, producer_no, Lady_Image):
    ...
    while (left):
        ...
        if (X_cord >= Producer_Left_Extreme):
            Lady_Image = pygame.image.load(
                ProImageL+str(image_id).zfill(2)+".png")
            Lady_Image = pygame.transform.scale(
                Lady_Image, (368/PROD_RATIO, 720/PROD_RATIO))
            X_cord = X_cord - PROD_SPEED
        else:
...
def message_to_screen(msg, color, x_displace=-50, y_displace=0, size='small'):
    ...
    textRect.center = (display_width/2) + \
        x_displace, (display_height/2)+y_displace
...
def StartScreen():
    ...
    while intro:
        ...
        message_to_screen('Producer-Consumer-Synchronization-Problem',
                          green, y_displace=50, size='medium')
        message_to_screen('Press any Key to start!', red,
                          y_displace=250, size='small')
        message_to_screen('Press q to quit!', red,
                          y_displace=300, size='small')
